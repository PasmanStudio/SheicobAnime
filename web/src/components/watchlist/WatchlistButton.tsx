...
export default function WatchlistButton({ seriesSlug, seriesTitle, coverUrl }: Props) {
  ...
  if (status === "loading") {
    return <div className="h-9 w-28 rounded-lg bg-neutral-800 animate-pulse" />;
  }
  ...
  const colorClass = currentStatus ? WATCH_STATUS_COLORS[currentStatus] : "text-neutral-300 border-neutral-700";
    ...
    <div className="relative inline-block" ref={ref}>
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm font-medium transition-colors
          hover:bg-neutral-800 disabled:opacity-60 ${colorClass}`}
      ...
      {open && (
        <div
          className="absolute z-50 left-0 top-full mt-1 w-40 rounded-xl bg-neutral-900 border border-neutral-700 shadow-2xl overflow-hidden"
          role="listbox"
        >
          {WATCH_STATUSES.map((s) => (
            <button
              ...
              className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm transition-colors
                ...
                ${currentStatus === s ? "text-white font-semibold" : "text-neutral-300"}`}
            >
          ...
          {currentStatus && (
            <>
              ...
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2.5 text-sm text-neutral-500 hover:text-red-400 hover:bg-neutral-800 transition-colors"
              >
